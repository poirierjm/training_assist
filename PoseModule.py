...
class PoseDetector:
    ...
    def findAngle(self, img, p1, p2, p3, draw=True):
        ...
        if draw:
            ...
            anglePer = int(np.interp(angle, (85, 165), (100, 0)))
            cv2.putText(img, str(anglePer), (x2 - 50, y2 + 50),
                        cv2.FONT_HERSHEY_PLAIN, 3, (180, 180, 180), 3)
            # jaune 0 255 255
            # rouge 0 0 255
            # vert 0 255 0
            # angleColor1 = 0
            # if angle <= 125:
            #    angleColor1 = int(np.interp(angle, (85, 125), (255, 100)))
            # elif angle > 125:
            # angleColor1 = int(np.interp(angle, (125, 165), (125, 255)))
            # angleColor2 = int(np.interp(angle, (125, 165), (70, 140)))
            # 255 140
            # 150  80
            x11 = int(np.interp(angle, (85, 165), (x2, x1)))
            x33 = int(np.interp(angle, (85, 165), (x2, x3)))
            y11 = int(np.interp(angle, (85, 165), (y2, y1)))
            y33 = int(np.interp(angle, (85, 165), (y2, y3)))
            cv2.line(img, (x1, y1), (x2, y2), (160, 160, 160), 10)
            cv2.line(img, (x3, y3), (x2, y2), (160, 160, 160), 10)

            cv2.line(img, (x1, y1), (x11, y11), (85, 150, 0), 10)
            cv2.line(img, (x3, y3), (x33, y33), (85, 150, 0), 10)
