...
def putTextRect(img, text, pos, scale=3, thickness=3, colorT=(255, 255, 255),
                ...
                offset=10, border=None, colorB=(0, 255, 0), fill=cv2.FILLED):
    """
    ...
    if border is not None:
        cv2.rectangle(img, (x1, y1), (x2, y2), colorR, fill)
...
def drawArcCv2(color, center, radius, thickness, end_angle, img):
    circle_image = np.zeros((radius * 2 + 4, radius * 2 + 4, 4), dtype=np.uint8)
    cv2.ellipse(img, (radius + 2, radius + 2),
                (radius - thickness // 2, radius - thickness // 2), 0, 0, end_angle, (*color, 255), thickness,
